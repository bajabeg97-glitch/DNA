...
try:
    ...
    from .relationship_sequence_inference import RelationshipSequenceInferenceEngine
except Exception:
    RelationshipInferenceEngine=None
    RelationshipSequenceInferenceEngine=None
...
class MelodicRelationshipEngine:
    ...
    """
    def __init__(self,data_dir:str|Path,relationship_model_dir:str|Path|None=None,relationship_dataset_path:str|Path|None=None,sequence_model_dir:str|Path|None=None,sequence_dataset_path:str|Path|None=None):
        ...
        if RelationshipSequenceInferenceEngine is not None and sequence_model_dir and sequence_dataset_path:
            try:self.sequence_model=RelationshipSequenceInferenceEngine(sequence_model_dir,sequence_dataset_path)
            except Exception:self.sequence_model=None
        self.relationship_notice='REAL_ORIGINAL_MIDI_SEQUENCE_MODEL_V2' if self.sequence_model else ('REAL_ORIGINAL_MIDI_RELATIONSHIP_RANKER_V1' if self.relationship_ranker else 'PROJECT_RELATIONSHIP_BASELINE_NO_LEARNED_RANKER')
    ...
    def _sequence_action(self,pred:dict,variant:int)->str:
        probs=pred.get('actionProbabilities') or {}
        play=float(probs.get('PLAY',0)); skip=float(probs.get('SKIP',0)); hold=float(probs.get('HOLD',0))
        if variant==0:return pred.get('action','PLAY')
        if variant==1:return 'PLAY' if play>=.72 else ('HOLD' if hold>skip else 'SKIP')
        return 'SKIP' if skip>=.72 else ('HOLD' if hold>=.58 and hold>play else 'PLAY')

    def _third_notes_learned(self,midi:MidiFile,song:dict,req:MelodicRelationshipRequest,source:list[Note],preds:list[dict],variant:int,end:int):
        out=[];proof=[];skipped=0
        gate_scale=(1.0,.90,1.06)[variant]
        for i,(n,pred) in enumerate(zip(source,preds)):
            action=self._sequence_action(pred,variant)
            # Preserve phrase anchors against out-of-distribution all-SKIP predictions.
            if i==0 or i==len(source)-1 or n.start % max(1,midi.ppq)==0: action='PLAY'
            if action=='SKIP':skipped+=1;continue
            if action=='HOLD':
                if out: out[-1].end=min(end,max(out[-1].end,n.end))
                else: skipped+=1
                proof.append({'sourcePitch':n.pitch,'action':'HOLD','neural':pred})
                continue
            chord=self._chord_at(song,n.start)
            if not chord or float(chord.get('confidence',0))<.80:skipped+=1;continue
            interval=int(pred.get('interval',0)); scale=_SCALE_BY_QUALITY.get(str(chord.get('quality')))
            target=n.pitch+interval; root=int(chord.get('root',0)) if chord else 0
            learned_valid=abs(interval) in (3,4) and 0<=target<=127 and scale is not None and ((target-root)%12 in scale)
            if not learned_valid:
                interval=self._third_interval(n.pitch,chord)
                if interval is None:skipped+=1;continue
                target=n.pitch+interval
            ratio=max(.35,min(1.25,float(pred.get('durationRatio',1.0))))*gate_scale
            note_end=min(end,n.start+max(1,int(round((n.end-n.start)*ratio))))
            v=self.velocity.resolve(midi,req.target_track_index,req.target_channel,n.start,'third')
            out.append(Note(req.target_track_index,req.target_channel,target,n.start,note_end,v['velocity'],factory_profile_id=v['profileId'],element='third'))
            proof.append({'sourcePitch':n.pitch,'targetPitch':target,'interval':interval,'action':'PLAY','learnedIntervalAccepted':learned_valid,'chord':chord.get('symbol'),'neural':pred,'factory':v})
        return out,proof,skipped

    def _echo_notes_learned(self,midi:MidiFile,song:dict,req:MelodicRelationshipRequest,source:list[Note],preds:list[dict],variant:int,end:int):
        out=[];proof=[];skipped=0
        delay_scale=(.86,1.0,1.14)[variant]; gate_scale=(1.05,.88,.72)[variant]
        ioi=self._median_ioi(source,midi.ppq)
        for i,(n,pred) in enumerate(zip(source,preds)):
            action=self._sequence_action(pred,variant)
            if i==0 or i==len(source)-1 or n.start % max(1,midi.ppq)==0: action='PLAY'
            if action=='SKIP':skipped+=1;continue
            if action=='HOLD':
                ...
                proof.append({'sourcePitch':n.pitch,'action':'HOLD','neural':pred})
                continue
            dq=max(.10,min(1.5,float(pred.get('delayQn',.5))))*delay_scale
            delay=max(1,min(int(round(dq*midi.ppq)),max(1,int(ioi*.78))))
            start=n.start+delay
            if start>=end:skipped+=1;continue
            next_start=source[i+1].start if i+1<len(source) else end
            if start>=next_start:skipped+=1;continue
            ratio=max(.18,min(.95,float(pred.get('durationRatio',.5))))*gate_scale
            note_end=min(end,next_start,start+max(1,int(round((n.end-n.start)*ratio))))
            if note_end<=start:skipped+=1;continue
            v=self.velocity.resolve(midi,req.target_track_index,req.target_channel,start,'echo')
            out.append(Note(req.target_track_index,req.target_channel,n.pitch,start,note_end,v['velocity'],factory_profile_id=v['profileId'],element='echo'))
            proof.append({'sourcePitch':n.pitch,'action':'PLAY','delayTicks':delay,'durationRatio':ratio,'neural':pred,'factory':v})
        return out,proof,skipped
    ...
    def generate(self,midi_bytes:bytes,req:MelodicRelationshipRequest)->dict[str,Any]:
        ...
        variants={}
        sequence_predictions=self.sequence_model.predict(source,req.kind,midi.ppq) if self.sequence_model and req.kind in {'third','echo'} else None
        for idx,label in enumerate('ABC'):
            if req.kind=='third' and sequence_predictions is not None: notes,proof,skipped=self._third_notes_learned(midi,song,req,source,sequence_predictions,idx,end)
            elif req.kind=='echo' and sequence_predictions is not None: notes,proof,skipped=self._echo_notes_learned(midi,song,req,source,sequence_predictions,idx,end)
            elif req.kind=='third': notes,proof,skipped=self._third_notes(midi,song,req,source,idx,end)
            ...
            continuity_role={'third':'third','echo':'echo','solo-performance':'solo'}.get(req.kind,req.kind)
            before_metrics=continuity_metrics(notes,ppq=midi.ppq)
            notes,continuity=smooth_phrase_notes(notes,ppq=midi.ppq,role=continuity_role,region_end=end)
            continuity['before']=before_metrics; continuity['after']=continuity_metrics(notes,ppq=midi.ppq)
            ...
            variants[label]={'midiBytes':raw,'sha256':hashlib.sha256(raw).hexdigest(),'noteCount':len(target),'proof':proof,
                             'skipped':skipped,'continuityGuard':continuity,'factoryVelocityProfileIds':factory_ids,'factorySoundExactForAll':exact,
                             'sourcePitchPreserved':req.kind!='third','goldVelocityUsed':False,'neuralVelocityUsed':False,
                             'protectedEventsPreserved':True,'relationshipModel':learned,'sequenceModelUsed':bool(sequence_predictions)}
        ...
        return {'schema':'dna-melodic-relationship-engine','version':'1.2','request':asdict(req),'sourceSha256':hashlib.sha256(midi_bytes).hexdigest(),
