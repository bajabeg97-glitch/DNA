...
from .instrument_behavior import solo_ornament_behavior_score
from .instrument_performance_grammar import performance_intent
...
def plan_solo_enhancement(
    ...
    for index, (current, following) in enumerate(zip(originals, originals[1:])):
        ...
        if not eligible:
            ...
            continue
        # Preserve phrase-level ornament diversity, then rank evidence within
        # the chosen musical function. This avoids a greedy model collapsing
        # the whole solo into one ornament type.
        available_kinds = [kind for kind in ("trill", "grace", "slide") if any(item.kind == kind for item in eligible)]
        phrase_pos = index / max(1, len(originals) - 2)
        intent = performance_intent("solo", phrase_position=phrase_pos, section_energy=config.intensity / 100.0)
        preferred_tokens = " ".join(intent.get("preferred", [])).lower()
        grammar_kinds = [kind for kind in available_kinds if kind in preferred_tokens]
        # Grammar narrows the choice when it has a clear phrase-state preference;
        # deterministic rotation preserves diversity among the musically valid kinds.
        pool_kinds = grammar_kinds or available_kinds
        preferred_kind = pool_kinds[index % len(pool_kinds)] if pool_kinds else None
        ranked_pool = [item for item in eligible if item.kind == preferred_kind] if preferred_kind else eligible
        ranked_pool.sort(
            key=lambda item: (
                -(item.confidence + 0.18 * solo_ornament_behavior_score(item.kind, current, following, midi.ppq)),
                item.evidence_id,
            )
        )
        evidence = ranked_pool[0]
